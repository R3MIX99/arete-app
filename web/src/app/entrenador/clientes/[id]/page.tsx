...
interface AssignmentOverrideRow {
...
interface FullAssignmentRow {
  ...
  programs:
    ...
    | null;
  routines: { id: string; name: string } | { id: string; name: string }[] | null;
  assignment_overrides: AssignmentOverrideRow[] | null;
}
...
export default async function ClientDetailPage({
  ...
  // semanas + reemplazos puntuales) para saber qué tenía programado
  // cada día, no solo lo que sí completó — se usa en la pestaña
  // Asistencia para los días que se le pasaron sin entrenar.
  const { data: fullAssignmentRows } = await supabase
    .from("client_assignments")
    .select(
  ...
  const assignments: CalendarAssignment[] = ((fullAssignmentRows ?? []) as FullAssignmentRow[]).map(
    (row) => {
      const program = one(row.programs);
      const routine = one(row.routines);
